...
//    return true;
    if(effects == null){
      Log.e(TAG, "EnableBuiltInAEC: is not supported");
      return false;
    }
  ...
  private int InitRecording(int sampleRate, int channels) {
    ...
    if(!WebRtcAudioUtils.hasPermission(context, android.Manifest.permission.RECORD_AUDIO)){
      Log.e(TAG, "InitRecording: RECORD_AUDIO permission is missing");
      return -1;
    }
    if(audioRecord != null){
    ...
    /* add by tannzh 2016-08-31 */
    boolean bAECSupported = effects.canUseAcousticEchoCanceler();
    boolean bAGCSupported = effects.canUseAutomaticGainControl();
    boolean bNSSupported = effects.canUseNoiseSuppressor();

    Log.d(TAG, "InitRecording: AEC support: " + bAECSupported + ", " +
          "AGC support: " + bAGCSupported + ", " +
          "NS support: " + bNSSupported);
